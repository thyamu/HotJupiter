...
def highest_predictor(data, rank):
    '''
    To identify given rank highest values
    :param data:
    :param rank:
    :return:
    '''
...
for spread in ["50", "250", "500"]:
    ...
    for feature_group in individual_features:
        input_path = ml_dir + "accuracy_%s.json"%feature_group
    ...
    for temp_index in range(len(list_temp)):
        ...
        best_features[spread][list_temp[temp_index]] = list()
        for x in dict_temp_feature[temp_index].keys():
            if dict_temp_feature[temp_index][x] < selected_value:
                continue
...
for spread in ["50", "250", "500"]:  # spread
    ...
    for t in np.arange(400, 2100, 100):
        ...
        features = best_features[spread][t] + ['Kzz']
